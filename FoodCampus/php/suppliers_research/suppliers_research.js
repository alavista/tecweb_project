            ...
            if (suppliers.status.localeCompare("error") == 0) {
                ...
                if (suppliers.data.length === 0) {
                    ...
                    for(var i = 0; i < suppliers.data.length; i++) {
                        ...
                        + "<p id='averageRating" + i + "'><strong>" + ((suppliers.data[i]["valutazione_media"] === null) ? "/" : suppliers.data[i]["valutazione_media"]) + "</strong> su 5 stelle (" + suppliers.data[i]["nrec"] + " voti)</p>"
                        + "</div>";
                    }
